...
class RecursoTAController extends Controller
{
        ...
        if ($request->hasFile('fotos')) {
            ...
            foreach ($fotosCarregadas as $key => $foto) {
                ...
                if (!Storage::disk('public')->has('uploads')) {
                    Storage::disk('public')->makeDirectory('uploads', 0o775, true, true);
                }
                ...
                if (!Storage::disk('public')->has('thumbnails')) {
                    Storage::disk('public')->makeDirectory('thumbnails', 0o775, true, true);
                }
    ...
    private function tryCreateContributionUploadDir()
    {
        if (!Storage::disk('public')->has('contribute_uploads')) {
            Storage::disk('public')->makeDirectory('contribute_uploads', 0o775, true, true);
        }
    }
    ...
    private function enviaEmailAdminContribuicao($nomeContribuicao)
    {
        $admin = User::findOrFail(1);
        $data = ['contribuicao' => $nomeContribuicao];

        Mail::send('emailAdminContribuicao', $data, function ($message) use ($admin) {
            $message->to($admin->email, $admin->name)
                ->subject('Nova contribuição registrada no RETACE');
            $message->from($admin->email, 'RETACE');
        });

        return 'Mensagem de nova contribuição enviada com sucesso';
    }
